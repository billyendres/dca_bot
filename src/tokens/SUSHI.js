...
const SUSHI = () => {
  ...
  useEffect(() => {
    setInterval(() => {
      const calclateProfitAndLoss = async () => {
        ...
        setProfitAndLoss(profit[1].unRealizedProfit);
        console.log(profit);
      };
  ...
  useEffect(() => {
    setInterval(() => {
      const coinPrices = async () => {
        ...
        setPrice(tokenPrice.SUSHIUSDT);
        console.log(tokenPrice.SUSHIUSDT);
      };
      coinPrices();
    }, 1000);
    // console.log(price);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        ...
        setPositionSize(pos[1].positionAmt);
        console.log(pos);
      };
      size();
    }, 1000);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        let pos = await binance.futuresPositionRisk();
        setAveragePrice(pos[1].entryPrice);
      };
      size();
    }, 1000);
  }, []);
