...
const WAVES = () => {
  ...
  useEffect(() => {
    setInterval(() => {
      const calclateProfitAndLoss = async () => {
        ...
        setProfitAndLoss(profit[10].unRealizedProfit);
        console.log(profit);
      };
  ...
  useEffect(() => {
    setInterval(() => {
      const coinPrices = async () => {
        ...
        setPrice(tokenPrice.WAVESUSDT);
        // console.log(tokenPrice.WAVESUSDT);
      };
      coinPrices();
    }, 1000);
    // console.log(price);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        ...
        setPositionSize(pos[10].positionAmt);
        console.log(pos);
      };
      size();
    }, 1000);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        let pos = await binance.futuresPositionRisk();
        setAveragePrice(pos[10].entryPrice);
      };
      size();
    }, 1000);
  }, []);
