...
const LRC = () => {
  ...
  useEffect(() => {
    setInterval(() => {
      const calclateProfitAndLoss = async () => {
        ...
        setProfitAndLoss(profit[5].unRealizedProfit);
        console.log(profit);
      };
  ...
  useEffect(() => {
    setInterval(() => {
      const coinPrices = async () => {
        ...
        setPrice(tokenPrice.ENJUSDT);
        // console.log(tokenPrice.ENJUSDT);
      };
      coinPrices();
    }, 1000);
    // console.log(price);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        ...
        setPositionSize(pos[5].positionAmt);
        console.log(pos);
      };
      size();
    }, 1000);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        let pos = await binance.futuresPositionRisk();
        setAveragePrice(pos[5].entryPrice);
      };
      size();
    }, 1000);
  }, []);
