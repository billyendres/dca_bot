...
const Ont = () => {
  ...
  const [takeProfitPercentage, setTakeProfitPercentage] = useState(0.003);
  ...
  useEffect(() => {
    setInterval(() => {
      const calclateProfitAndLoss = async () => {
        ...
        setProfitAndLoss(profit[12].unRealizedProfit);
      };
  ...
  useEffect(() => {
    setInterval(() => {
      const coinPrices = async () => {
        ...
        setPrice(tokenPrice.NEARUSDT);
        console.log(tokenPrice.NEARUSDT);
      };
      coinPrices();
    }, 1000);
    // console.log(price);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        ...
        setPositionSize(pos[12].positionAmt * 10);
        console.log(pos);
      };
      size();
    }, 1000);
    ...
    const takeProfit = async () => {
      ...
      if (profitAndLoss > takeProfitPercentage && positionSize > 0) {
        await binance.futuresMarketSell(
          baseSymbol,
          positionSize,
          //   parseFloat(price * priceDeviationShort).toFixed(4),
          {
            reduceOnly: true,
          }
        );
      ...
      if (profitAndLoss > takeProfitPercentage && positionSize < 0) {
        await binance.futuresMarketBuy(
          baseSymbol,
          positionSize * -1,
          //   parseFloat(price * priceDeviationLong).toFixed(4),
          {
            reduceOnly: true,
          }
        );
