...
const SUSHI = () => {
  ...
  useEffect(() => {
    setInterval(() => {
      const calclateProfitAndLoss = async () => {
        ...
        setProfitAndLoss(profit[24].unRealizedProfit);
        console.log(profit);
      };
  ...
  useEffect(() => {
    setInterval(() => {
      const coinPrices = async () => {
        ...
        setPrice(tokenPrice.AAVEUSDT);
        console.log(tokenPrice.AAVEUSDT);
      };
      coinPrices();
    }, 1000);
    // console.log(price);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        ...
        setPositionSize(pos[24].positionAmt);
        console.log(pos);
      };
      size();
    }, 1000);
  ...
  useEffect(() => {
    setInterval(() => {
      const size = async () => {
        let pos = await binance.futuresPositionRisk();
        setAveragePrice(pos[24].entryPrice);
      };
      size();
    }, 1000);
  }, []);
  ...
  const initialBuy = async () => {
    await binance.futuresBuy(
      baseSymbol,
      quantity,
      parseFloat(price * 0.998).toFixed(3)
    );
  };
  ...
  const safetyOrderOneLong = async () => {
    await binance.futuresBuy(
      ...
      parseFloat(price * priceDeviationLong * priceDeviationLong).toFixed(3)
    );
  };
  ...
  const initialSell = async () => {
    await binance.futuresSell(
      baseSymbol,
      quantity,
      parseFloat(price * 1.002).toFixed(3)
    );
  };
  ...
  const safetyOrderOneShort = async () => {
    await binance.futuresSell(
      baseSymbol,
      quantity * dcaQuantity,
      parseFloat(price * priceDeviationShort * priceDeviationShort).toFixed(3)
    );
  };
    ...
    const takeProfit = async () => {
      ...
      if (positionSize > 0 && startBotLong) {
        await binance.futuresSell(
          baseSymbol,
          positionSize,
          parseFloat(takeProfitPriceLong).toFixed(3),
          {
            reduceOnly: true,
          }
        );
      ...
      if (positionSize < 0 && startBotShort) {
        await binance.futuresBuy(
          baseSymbol,
          positionSize * -1,
          parseFloat(takeProfitPriceShort).toFixed(3),
          {
            reduceOnly: true,
          }
        );
